...
23. 文字、图片、div 区域水平居中，都可以使用 text-align:center 进行设置。（**√** ） 
24. margin:1px 2px 1px 3px; 可以缩写成 margin:1px 2px 3px; （<span style="color:red;font-size:26px;">×</span>  ） 
25. 浮动定位通过 float 属性设置来实现。 （<span style="color:red;font-size:26px;">×</span>  ） 
26. 默认定位方式就是文档流定位。 （**√** ）
...
33. 通常将父元素设置为绝对定位，子元素设置为相对定 位，就可以将子元素相对于父元素定位了。 （<span style="color:red;font-size:26px;">×</span>  ）

### 多选题：
1. CSS 中，盒模型的属性包括（ABC ） 。
...
61. 阅读下面 HTML 代码，两个 div 之间的垂直空白距离是 （**D** ）。
     \<style type="text/css"> 
